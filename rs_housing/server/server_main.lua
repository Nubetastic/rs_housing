...
local function LoadProperties()
    ...
    for _, property in pairs(Config.Properties) do
        ...
        exports["ghmattimysql"]:execute("SELECT * FROM `properties` WHERE name = @name", { ["@name"] = _ }, function(result)
            ...
            -- FIX: marcar como cargado solo cuando TODAS las queries han terminado
            loaded = loaded + 1
...
AddEventHandler("rs_housing:server:buySelectedProperty", function(actionType, propertyId, target)
    ...
    if not IsPermittedToBuy(citizenid) then
        TriggerClientEvent('rs_housing:ShowAdvancedNotification', _tsource, Locales['HOUSING_NOTI'], Locales['REACHED_MAX_PROPERTIES'], "menu_textures", "cross", 3000, "COLOR_RED")
        if target then
            TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], Locales['TARGET_REACHED_MAX_PROPERTIES'], "menu_textures", "cross", 3000, "COLOR_RED")
        end
        return
    end
    ...
    if not hasEnough then
        TriggerClientEvent('rs_housing:ShowAdvancedNotification', _tsource, Locales['HOUSING_NOTI'], Locales['NOT_ENOUGH_MONEY'], "menu_textures", "cross", 3000, "COLOR_RED")
        if target then
            TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], Locales['TARGET_NOT_ENOUGH_MONEY'], "menu_textures", "cross", 3000, "COLOR_RED")
        end
        return
    end
    ...
    if target and Config.RealEstateJob.Enabled and Config.RealEstateJob.Job == PlayerData.job then
        ...
        TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], string.format(notifyData, rewardAmount), "generic_textures", "tick", 3000, "COLOR_GREEN")
    end
    ...
    TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], Locales['SUCCESSFULLY_BOUGHT_PROPERTY'], "generic_textures", "tick", 3000, "COLOR_GREEN")
...
AddEventHandler("rs_housing:server:sell", function(propertyId)
    ...
    if receiveAmount > 0 then
        ...
        TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], string.format(Locales['SOLD_PROPERTY_RECEIVED_DOLLARS'], receiveAmount), "generic_textures", "tick", 3000, "COLOR_GREEN")
    else
        TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], Locales['SOLD_PROPERTY'], "generic_textures", "tick", 3000, "COLOR_GREEN")
    end
...
AddEventHandler('rs_housing:server:updateAccountLedgerById', function(propertyId, transactionType, amount)
    ...
    if transactionType == 'DEPOSIT' and currentMoney < amount then
        TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], Locales['NOT_ENOUGH_MONEY_LEDGER_DEPOSIT'], "menu_textures", "cross", 3000, "COLOR_RED")
        return
    end

    if transactionType == 'DEPOSIT' and propertyData.tax ~= nil then
        if (propertyData.ledger + amount) > propertyData.tax then
            TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], Locales['MONEY_LEDGER_DEPOSIT_LIMIT'], "menu_textures", "cross", 3000, "COLOR_RED")
            return
        end
    end
    ...
    if transactionType == 'DEPOSIT' then
        ...
        TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], string.format(Locales['LEDGER_DEPOSITED_ACCOUNT_MONEY'], amount), "generic_textures", "tick", 3000, "COLOR_GREEN")
    end
...
AddEventHandler("rs_housing:server:transferOwnedProperty", function(propertyId, targetSource)
    ...
    if not IsPermittedToBuy(citizenid) then
        TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], Locales['TARGET_REACHED_MAX_PROPERTIES'], "menu_textures", "cross", 3000, "COLOR_RED")
        TriggerClientEvent('rs_housing:ShowAdvancedNotification', targetSource, Locales['HOUSING_NOTI'], Locales['REACHED_MAX_PROPERTIES'], "menu_textures", "cross", 3000, "COLOR_RED")
        return
    end
    ...
    TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], Locales['TRANSFERRED_PROPERTY'], "generic_textures", "tick", 3000, "COLOR_GREEN")
    TriggerClientEvent('rs_housing:ShowAdvancedNotification', targetSource, Locales['HOUSING_NOTI'], Locales['TRANSFERRED_PROPERTY_TO_SELF'], "generic_textures", "tick", 3000, "COLOR_GREEN")
...
AddEventHandler('rs_housing:server:updateAccountLedgerHomeById', function(propertyId, transactionType, amount)
    ...
    if transactionType == 'WITHDRAW' and propertyData.ledgerhome < amount then
        TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], Locales['NOT_ENOUGH_MONEY_LEDGER_WITHDRAW'], "menu_textures", "cross", 3000, "COLOR_RED")
        return
    end
    ...
    if transactionType == 'DEPOSIT' then
        ...
        Properties[propertyId].ledgerhome = Properties[propertyId].ledgerhome - amount
        TriggerClientEvent('rs_housing:ShowAdvancedNotification', _source, Locales['HOUSING_NOTI'], string.format(Locales['LEDGER_WITHDREW_ACCOUNT_MONEY'], amount), "generic_textures", "tick", 3000, "COLOR_GREEN")
    end
